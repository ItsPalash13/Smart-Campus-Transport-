...
const busIcon = require('@/assets/images/busicon1.png'); // Bus icon path
...
function CustomDrawerContent({ user, auth, selectedBus, setSelectedBus, navigation, stopTrackingLocation, removeRouteFromSelectedBus }) {
  const handleLogout = async () => {
    try {
      // Stop tracking location when logging out
      stopTrackingLocation();

      // Unselect the bus if a bus is selected
      if (selectedBus) {
...
function AuthenticatedScreen({ user, auth }) {
  ...
  const sendGeolocationToFirebase = async () => {
    if (!selectedBus || !user) {
      ...
      return;  // Exit the function if user is logged out or bus is not selected
    }
  ...
  const startTrackingLocation = async () => {
    try {
      ...
      // Check if a bus and destination are selected
